...
export class RestManagementApp extends HandlebarsApplicationMixin(ApplicationV2) {
  ...
  _onRender() {
    ...
    root.querySelectorAll('[name="keeper.prepareSpells"]').forEach(input => {
      input.addEventListener("change", () => this.#refreshPreparedSpellOptions());
    });
  ...
  #payloadFor(action) {
    ...
    switch (action.kind) {
      ...
      case "prepare-spells": {
        const section = root.querySelector("[data-spell-preparation]");
        const classItemId = String(section?.dataset?.classItemId ?? action.classItemId ?? "");
        const limit = Math.max(0, Number(section?.dataset?.preparedLimit ?? 0));
        const preparedSpellItemIds = checkedValues("keeper.prepareSpells");
        if (!classItemId) throw new Error("The class for this prepared-spell choice is unavailable.");
  ...
  #refreshPreparedSpellOptions() {
    const root = this.element;
    if (!root) return;
    const section = root.querySelector("[data-spell-preparation]");
    if (!section) return;
    const limit = Math.max(0, Number(section.dataset.preparedLimit ?? 0));
    const inputs = [...section.querySelectorAll('[name="keeper.prepareSpells"]')];
    const selected = inputs.filter(input => input.checked).length;
    const atLimit = selected >= limit;
    for (const input of inputs) {
      input.disabled = !input.checked && atLimit;
      const card = input.closest(".cb-prepare-spell-card");
