...
export class WarBondManagementService {
  ...
  static activity(feature, activityId = null) {
    ...
    if (explicit && this.matchesActivity(explicit, feature) && this.#isNativeEnchantActivity(explicit)) return explicit;
    return activities.find(activity => this.matchesActivity(activity, feature) && this.#isNativeEnchantActivity(activity)) ?? null;
  }

  /**
   * Match only the source-native Bond with Weapon activity on the War Bond feature.
   * D&D5e calls preUseActivity with an Activity belonging to a cloned Item, so
   * the matcher intentionally relies on stable feature/activity identity rather
   * than assuming the original embedded Item instance is still attached.
   */
  static matchesActivity(activity, feature = null) {
    const item = feature ?? activity?.item ?? activity?.parent?.parent ?? activity?.parent ?? null;
  ...
  /** Resolve the live Actor behind a preUseActivity clone. */
  static actorForActivity(activity) {
    const direct = activity?.actor ?? activity?.item?.actor ?? activity?.item?.parent ?? null;
    if (direct?.type === "character" && direct?.items) return direct;

    const itemId = activity?.item?.id ?? activity?.item?._id ?? null;
    if (!itemId) return null;

    const candidates = [];
    const seen = new Set();
    const add = actor => {
      if (!actor || actor.type !== "character" || seen.has(actor.uuid ?? actor.id)) return;
      seen.add(actor.uuid ?? actor.id);
      candidates.push(actor);
    };

    for (const token of globalThis.canvas?.tokens?.placeables ?? []) add(token?.actor);
    for (const actor of globalThis.game?.actors?.contents ?? globalThis.game?.actors ?? []) add(actor);

    return candidates.find(actor => {
      const item = actor?.items?.get?.(itemId);
      return item && (this.#identifier(item) === "war-bond" || this.#normalize(item.name) === "war bond");
    }) ?? null;
  }
  ...
  static #isNativeEnchantActivity(activity) {
    const type = String(activity?.type ?? "").trim().toLowerCase();
    if (type === "enchant") return true;
    return typeof activity?.applyEnchantment === "function"
      && typeof activity?.canEnchant === "function"
      && activity?.availableEnchantments != null;
  }
