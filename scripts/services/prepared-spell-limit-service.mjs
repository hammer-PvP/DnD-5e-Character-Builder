...
/**
 * Enforces ordinary prepared-spell list limits without treating spell
 * preparation timing as build provenance. Access model and preparation cadence
 * are intentionally separate; this service owns the numeric list cap only.
 * It manages ordinary prepared state (1). Always Prepared and feature-granted
 * spells are never counted or
 * auto-unprepared.
 */
export class PreparedSpellLimitService {
  ...
  /**
   * Resolve the prepared-list class that owns a spell. Character Builder
  ...
  static classContextForSpell(actor, spell) {
    ...
    const cls = this.owningClassForSpell(actor, spell);
    if (!cls || cls.type !== "class" || !this.#usesPreparedList(cls)) return null;
    const limit = this.maxPrepared(cls);
    if (!Number.isFinite(limit) || limit <= 0) return null;
    return { cls, limit };
  }

  /** Resolve the class provenance of a spell without conflating it with access model or cadence. */
  static owningClassForSpell(actor, spell) {
    if (!actor || spell?.type !== "spell") return null;
    ...
    return cls?.type === "class" ? cls : null;
  }
  ...
  /** All ordinary level 1+ spells that belong to one prepared-list class. */
  static ordinaryClassSpells(actor, cls) {
    if (!actor || !cls) return [];
    return [...(actor.items ?? [])].filter(spell => {
      if (spell?.type !== "spell") return false;
      if (Number(spell.system?.level ?? 0) <= 0) return false;
  ...
  /** Classes whose ordinary spell Items use a bounded prepared list. */
  static preparedListClasses(actor) {
    return [...(actor?.items ?? [])].filter(item => item?.type === "class" && this.#usesPreparedList(item));
  }
  ...
  static isExcludedGrant(spell) {
    return this.#excludedGrant(spell);
  }

  static classByIdentifier(actor, identifier) {
    return this.#classByIdentifier(actor, identifier);
  }

  static accessModelForClass(cls) {
  ...
  static async reconcileActor(actor) {
    ...
    const classes = [...(actor.items ?? [])].filter(item => item?.type === "class" && this.#usesPreparedList(item));
    const updates = [];
    const details = [];
  ...
  static #excludedGrant(spell) {
    ...
    const categories = new Set([
      flags.category,
      flags.levelUpSpell?.category,
      flags.featureAcquisition?.category,
      ...owners.map(owner => owner?.category)
    ].map(value => String(value ?? "").trim().toLowerCase()).filter(Boolean));

    // Savant grants are acquisitions into the Wizard spellbook, not Always
    // Prepared feature spells. They remain ordinary preparation candidates and
    // count against the Wizard's prepared-spell limit.
    if (categories.has("wizard-savant")) return false;

    if (flags.featureGrantedSpell === true || owners.length > 0) return true;
