 ...
 * maintenance-ready for the future Character Keeper without exposing any rest
...
export class PactOfTheTomeService {
  ...
  static async buildContext(actor, registry, {
    ...
    for (const option of preferred) {
      ...
      const disabledReason = !currentOwned ? unavailable.get(option.identifier) ?? "" : "";
      const row = {
        ...option,
        level,
  ...
  static async apply(actor, registry, {
    ...
    let book = actor.items.find(item => item.getFlag(MODULE_ID, "pactOfTheTomeBook")?.invocationItemId === invocation.id) ?? null;
    let created = [];
    ...
    try {
      ...
      try {
        await AdvancementChoiceAnnotationService.refresh(actor, {
          state: {
            context: mode === "acquisition" && String(transactionId ?? "").startsWith("creation:") ? "creation" : "levelUp",
            transactionId,
  ...
  static #validate(context, selectedCantrips, selectedRituals) {
    ...
    for (const identifier of cantrips) {
      ...
      if (!option || option.disabled) throw new Error(`${option?.name ?? identifier} is not eligible for Pact of the Tome.`);
    }
