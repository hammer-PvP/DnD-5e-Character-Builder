...
Hooks.once("init", async () => {
  ...
  Hooks.on("dnd5e.preUseActivity", (activity, usageConfig = {}) => {
    ...
    const actor = WarBondManagementService.actorForActivity(activity);
    if (!WarBondManagementService.canManage(actor)) return;
    const liveFeature = WarBondManagementService.feature(actor, activity?.item?.id ?? null);
    const liveActivity = WarBondManagementService.activity(liveFeature, activity?.id ?? activity?._id ?? null);
    if (!liveFeature || !liveActivity) return;
    setTimeout(() => {
      void WarBondManagerApp.launch(actor, {
        featureItemId: liveFeature.id,
        activityId: liveActivity.id ?? liveActivity._id ?? null
      });
    }, 0);
    return false;
  });
