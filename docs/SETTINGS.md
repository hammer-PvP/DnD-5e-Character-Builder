...
This reference documents every visible Character Builder setting in v0.9.9x2. Unless stated otherwise, settings are **world settings**, can be changed only by a Game Master, and do not require a server restart. Saving the settings window affects future Character Creation, Level Up, Character Keeper, or runtime-assistance operations; it does not retroactively delete character content.
...
- **Configure Integrity Rules:** Opens a separate GM configuration window with seven broad checkboxes plus the unprepared-spell casting policy. Every control includes a short description of exactly what it protects and what normal gameplay remains available.
- **Recommended defaults:** Restore Recommended enables all seven protection packages and selects **Out of Combat Only** for unprepared-spell casting. **Hit Point Fields** is off by default on a normal install/update so a GM deliberately chooses whether players may type HP values themselves. Existing worlds retain the previous master-switch state.
...
- **Hit Point Fields:** Disabled by default. When enabled, protected players cannot directly type/edit Current HP, Maximum HP, Temporary HP, or Temporary HP Maximum on the character sheet. It intentionally does not add a global Actor-update veto, so native D&D5e damage, healing, Hit Dice, rests, Activities, automation, GM edits, and authorized Character Builder updates remain available.
- **Allow Casting Unprepared Spells:** Available only inside **Configure Integrity Rules**. **On** allows ordinary unprepared level 1+ spells; **Out of Combat Only** permits them only outside a started Combat; **Off** requires preparation everywhere. This casting policy is independent from the Character Keeper preparation-edit lock. Wizard Ritual Adept retains the qualifying out-of-combat ritual exception when casting is Off.
...
Most rules are enabled by default inside the saved rule set, but do nothing while the master switch is off. Explicit homebrew rules may default off; **Healing Potion: Maximum Healing as Action** is opt-in.
...
- **Ranger — Primal Companion:** Completes only the observed native summon lifecycle gaps. A newly summoned Beast is set to its already-derived maximum current HP when necessary, receives the Ranger Actor's ownership through a Ranger-specific managed Actor, and replaces only that Ranger's prior managed Primal Companion. Native AC, PB, Beast's Strike, damage, maximum HP, Command, and Restore Companion remain D&D5e-owned.
- **Homebrew — Healing Potion: Maximum Healing as Action:** **Off by default.** Eligible Healing Potions keep their native Bonus Action Healing Activity and receive one flagged Character Builder-managed Action Healing Activity with each numeric die maximized through Foundry's native `minN` syntax. Official PHB/D&D5e potions are auto-recognized only when source identity is reliable. **Configure Potions** accepts World/Compendium Items by drag-and-drop for third-party/homebrew support and asks the GM to choose the source Healing Activity when necessary.
