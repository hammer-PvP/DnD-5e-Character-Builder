...
At **Finish Character**, Character Builder also reconciles D&D5e 5.3.3 Tool-choice rollback residue. A Tool mapping newly materialized by a temporary TraitAdvancement choice and later reversed is removed only when the final Draft no longer references it, it did not exist before the transaction, and it contains only the default proficiency-0 structure. Pre-existing or customized Tool entries are restored/preserved rather than guessed from `value` alone.
...
Current validation includes structural Activity/Active Effect links, missing deterministic Advancement Items, stale copied Actor references, dependent Advancement completion, class/subclass grants, Weapon Mastery, Fighting Style, spell access/repertoire and grant ownership, malformed empty Spell placeholders, and source-accounted Traits such as skills, Expertise, saves, languages, tools, weapon/armor training, damage resistances/immunities/vulnerabilities, and condition immunities. For Tools specifically, the Validator also checks final Advancement/source references before offering a safe repair for an unreferenced default proficiency-0 mapping; customized ambiguous entries are preserved for review. Equipment, money, ammunition quantities, current HP, spent spell slots, and spent resource amounts are intentionally outside validation scope.
...
**Concentration & Dependent Effects** keeps D&D5e authoritative for concentration documents and target-effect cleanup. A Concentration roll is kept pending until the shared post-roll queue reaches its final total, including Character Builder and Item-origin providers. If the final result still fails, Character Builder leaves concentration active and posts a GM decision card in Chat. **Keep Concentration** preserves the current state; **Drop Concentration** calls the native `Actor.endConcentration()` API, after which D&D5e removes dependent effects normally. For World-Time expiry, Foundry remains the sole finite-duration authority; if it marks the concentrating effect expired but leaves that live concentration marker behind, Character Builder re-checks the completed native state and invokes `Actor.endConcentration()` once. Character Builder also corrects the native concentration-request edge case where clicking a whispered DC request while another non-concentrating Actor is targeted would otherwise roll that wrong Actor. Non-concentration effects remain unaffected.
...
This reference documents every visible Character Builder setting in v0.9.9x11. Unless stated otherwise, settings are **world settings**, can be changed only by a Game Master, and do not require a server restart. Saving the settings window affects future Character Creation, Level Up, Character Keeper, or runtime-assistance operations; it does not retroactively delete character content.
...
- **Concentration & Dependent Effects:** After all Character/Item post-roll providers resolve, a final failed Concentration save does **not** immediately end concentration. Character Builder posts a GM-only decision in Chat. **Keep Concentration** preserves the effect; **Drop Concentration** calls native `Actor.endConcentration()`, after which D&D5e handles dependent effects normally. A post-roll bonus that turns the save into a success never creates the decision card. Foundry VTT 14 remains authoritative for finite World-Time expiry; Character Builder only finalizes a still-live concentration marker after Foundry has already marked that concentrating effect expired.
...
Character Builder `0.9.9x11` includes an automatic compatibility adapter for the optional module **Rest Recovery 5e** (`rest-recovery`). It is not a dependency and no Character Builder setting is required.
