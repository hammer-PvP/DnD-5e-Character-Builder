...
- Reworked the main **Character Builder Settings** flow so sections use the full available width rather than two independent vertical column stacks that could leave large dead areas. Removed the duplicate main-screen **Allow Casting Unprepared Spells** selector; that policy now lives only inside **Configure Integrity Rules**, while the main screen retains only its compact Integrity status summary.
- Added the optional **Hit Point Fields** Sheet Integrity rule, disabled by default. When enabled, protected players cannot directly type/edit Current HP, Maximum HP, Temporary HP, or Temporary HP Maximum through the character sheet. GM edits and native damage/healing/rest/Activity updates remain untouched; this rule intentionally avoids a broad Actor-update guard that could intercept legitimate D&D5e gameplay.
- Added **Ranger — Primal Companion** to Configure Assistance Rules. D&D5e remains authoritative for AC, Proficiency Bonus, Beast's Strike, damage, and maximum HP. Character Builder only completes the observed summon lifecycle gaps: a newly materialized Beast is SET to its already-derived maximum current HP when needed, inherits the Ranger Actor's ownership, is rebound to a Ranger-specific managed Actor, and replaces that Ranger's previous managed companion/Token without affecting another Ranger's companion. Orphaned D&D5e auto-import summon bases are removed only when no scene Token still references them.
- Added the opt-in **Homebrew — Healing Potion: Maximum Healing as Action** Assistance rule, disabled by default. Eligible Healing Potions retain their native Bonus Action Healing Activity and receive one Character Builder-managed Action Healing Activity whose numeric dice use Foundry's native `minN` modifier (`2d4 + 2` → `2d4min4 + 2`). Mixed numeric dice are maximized independently; dynamic/non-numeric die sizes are rejected rather than guessed.
- Added **Configure Potions** inside Configure Assistance Rules. Official Healing Potions with reliable PHB/D&D5e source identity are recognized automatically. Third-party/homebrew consumables can be registered by drag-and-drop from World Items or Compendiums; when multiple compatible Healing Activities exist, the GM chooses the intended source Activity. Disabling/removing eligibility deletes only the Character Builder-managed derived Activity and never the native Activity.
- Fixed Character Creation completeness for **idempotent duplicate fixed Trait/proficiency grants**. A mechanical proficiency such as Thieves' Tools may satisfy multiple canonical fixed sources (for example Criminal Background and Rogue) without forcing one source to “win” or blocking Finish Character. Human choice slots remain exclusive and mandatory, so a duplicate fixed grant cannot silently satisfy a real unresolved choice.
- Aligned the native Advancement navigation guard with the same Trait semantics: deterministic grants are validated mechanically by the final projection, while `value.chosen` is required only for genuine configured choice slots.
