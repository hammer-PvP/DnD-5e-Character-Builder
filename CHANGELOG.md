...
- Hardened the **Bond with Weapon** interception against D&D5e's `preUseActivity` clone lifecycle. The hook now resolves the live Actor/War Bond Item before launching **Manage War Bonds**, instead of depending on the cloned Activity retaining a usable Actor reference.
- Activity matching is anchored to the source-native **War Bond → Bond with Weapon** identity, while the live manager still requires the real native Enchant Activity/capabilities before applying a bond. **Summon Weapon** remains untouched and fully native.
- Preserved the x7 War Bond slot, Broken Bond, native enchantment, drag/drop, Break Bond, and native `uses.spent` reconciliation behavior.

### Distribution sanitation
- Reduced the installable `docs/` directory to a single user-facing `Character-Builder-Manual.md`.
- Removed duplicated documentation screenshots; GitHub/manual documentation now reuses the same `assets/tutorial/` files required by the in-game tutorial.
- Removed internal protocol/reference documents and the old Foundry-page HTML from the installable module package.
- Removed the unused `item-grant-reconciliation-service.mjs` compatibility alias; the active runtime uses `ItemGrantIntegrityService` directly.
- Kept runtime code, templates, language files, tutorial assets, README, CHANGELOG, LICENSE, and manifest files intact.

### Preserved validated x7 behavior
- Wizard **Memorize Spell** and Warlock **Fiendish Resilience** are unchanged from the live-validated x7 implementation.
...
- Compact **Character Keeper — Prepare Spells** into one functional header row: feature icon, `Prepare <Class> Spells`, prepared-count budget, and Confirm. Removed the redundant kicker/description and large explanatory Class Spell List block so Search and the spell list begin immediately below the header.
- Reworked the main **Character Builder Settings** flow so sections use the full available width rather than two independent vertical column stacks that could leave large dead areas. **Allow Casting Unprepared Spells** now exists only inside **Configure Integrity Rules**; the main settings screen retains the compact Integrity summary.
- Added the optional **Hit Point Fields** Sheet Integrity rule, disabled by default. When enabled, protected players cannot directly type/edit Current HP, Maximum HP, Temporary HP, or Temporary HP Maximum through the character sheet. GM edits and native damage/healing/rest/Activity updates remain untouched.
- Added **Ranger — Primal Companion** to Configure Assistance Rules. D&D5e remains authoritative for AC, Proficiency Bonus, Beast's Strike, damage, and maximum HP. After the native summon completes, Character Builder materializes the finalized synthetic Beast into a Ranger-specific linked Actor, SETS only that fresh Actor's current HP to the already-derived maximum when needed, inherits the Ranger's ownership, rebinds the placed Token without writing a partial ActorDelta, and removes only the previous companion belonging to that Ranger. Orphaned D&D5e auto-import summon templates are removed only when no scene Token references them.
- Added the opt-in **Homebrew — Healing Potion: Maximum Healing as Action** Assistance rule, disabled by default. Eligible Healing Potions retain their native Bonus Action Healing Activity and receive one Character Builder-managed Action Healing Activity whose numeric dice use Foundry's native `minN` modifier (`2d4 + 2` → `2d4min4 + 2`). This implementation is carried over from the live-validated behavior tested before the X3 rebase.
- Added **Configure Potions** inside Configure Assistance Rules. Official Healing Potions with reliable PHB/D&D5e source identity are recognized automatically. Third-party/homebrew consumables can be registered by drag-and-drop from World Items or Compendiums; when multiple compatible Healing Activities exist, the GM chooses the intended source Activity.
- Fixed Character Creation completeness for **duplicate deterministic Trait/proficiency grants** without assigning one source as the winner. Fixed grants are non-exclusive entitlements, and zero-option Trait choice rows are not counted as human choices. Thus Criminal Background and Rogue may both canonically grant Thieves' Tools while the Actor carries the mechanical proficiency once. Genuine non-empty choice pools remain mandatory and exclusive.
- Aligned the native Advancement navigation guard with the same semantics: fixed grants may be satisfied by the Actor's current mechanical state even when another source supplied it first, while real configured choice slots still require explicit resolution.
- No custom War Bond, Wizard Memorize Spell, Ranger Hunter maintenance, Bestial Fury/Exceptional Training automation, or workaround for the separately reported native `AdvancementManager._updatePosition` lifecycle error is included in this build.
...
- Added an authoritative `dnd5e.preUseActivity` guard for protected player-owned Actors. When the policy blocks an unprepared level 1+ class spell, the Activity is stopped before D&D5e opens its usage/upcast dialog, consumes a slot, or creates chat output, so sheet buttons, hotbar use, and macros share the same rule boundary.
- **Combat Only** permits the existing lenient out-of-combat workflow but blocks both use of unprepared spells and direct sheet preparation changes while the Actor is in a started Combat. **Always** blocks unprepared use everywhere and routes direct preparation changes to the class's rule-authorized preparation window.
- Preserved the Wizard **Ritual Adept** exception under Always: outside combat, an unprepared ritual-tagged Wizard spell that belongs to that Wizard's actual spellbook can be used as a slot-free, non-upcast ritual. Ritual Adept never bypasses the combat restriction, and a generic `ritual` casting method is not treated as permission by itself.
- Added **Long Rest Spell Preparation** to Character Keeper for **Cleric, Druid, Paladin, and Wizard**. Choices are staged before the rest and committed only after the authoritative native Long Rest completes; cancelling the native/Rest Recovery flow leaves the prepared list unchanged.
- Long Rest preparation is class-isolated for multiclass Actors. Cleric/Druid/Paladin use their own ordinary class spell pool, while Wizard uses only the level 1+ Wizard spells actually present in that Wizard's spellbook. Cantrips are excluded. Always Prepared and feature-granted spells are displayed as locked context and do not count against the ordinary prepared limit.
- Split **Spell Access Model** from **Spell Preparation Cadence**. Full-list access no longer implies Long Rest preparation: Ranger remains a full-list prepared caster for numeric limit/provenance purposes but its preparation cadence is Level Up, so it receives no Long Rest preparation action. Bard, Sorcerer, and Warlock likewise remain Level Up cadence.
- Corrected Wizard **Savant** handling in prepared-list classification: Savant spells are spellbook acquisitions, not feature-granted Always Prepared spells, so they remain legal Wizard preparation candidates and count against the normal Wizard prepared limit.
- The existing Prepared Spell Limit remains a separate numeric integrity rule from preparation timing. `0.9.9x` does not add Wizard Memorize Spell, custom War Bond handling, or new Ranger maintenance behavior.
- This `x` package is an internal test build. Public GitHub release URLs intentionally remain on the last public `0.9.9w` line until the next public release is approved.
