...
**Source-to-Target Damage Riders** extend contextual automation to damage that exists because a specific source Actor has marked or cursed a specific target. The runtime resolves the controller from the target effect, requires that controller to be the current attacker and that target to be the single selected target, then appends the source document's own Damage Activity to the native Attack damage process. Hunter's Mark uses `Bonus Mark Damage`, upgrading to Foe Slayer's native improved Activity when present; Hex uses `Bonus Hex Damage`. The formulas and damage types remain source-authored rather than hard-coded into weapons. If a supported source Activity declares native Active Effects but its usage message omits those effect references, Character Builder re-enters D&D5e through the spell's existing no-consumption helper so D&D5e's native Effects tray still presents and applies the choices. For Hex, the selected native `Hexed <Ability>` effect is additionally bound to the casting Actor/source Item when D&D5e creates it; if an intermediary card lost the concentration origin, Character Builder reconnects the target effect to the matching live native Concentration effect. This preserves Hex's six native ability-curse options without auto-selecting one and keeps the extra damage source-owned rather than weapon-authored.
